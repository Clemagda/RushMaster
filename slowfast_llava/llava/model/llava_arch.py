...
from slowfast_llava.llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN

from slowfast_llava.llava.mm_utils import get_anyres_image_grid_shape
...
class LlavaMetaForCausalLM(ABC):
    ...
    def temporal_aggregation(self, image_features, temporal_aggregation):
        ...
        if temporal_aggregation == "concat":
            ...
            image_features = pool2(image_features)
            image_features = rearrange(
                image_features, 'd t n0 n1 -> (t n0 n1) d')
    ...
    def prepare_slowfast(self, image_features, temporal_aggregation):
        ...
        slowfast_match = re.match(
            r'^slowfast-slow_(\d+)frms_(\w+)-fast_(\d+)x(\d+)$', temporal_aggregation)
        ...
        slowfast_features = torch.cat(
            (slowpath_features, fastpath_features), dim=1)
           ...
           temporal_aggregation.lower() != 'false':
            if temporal_aggregation.startswith('slowfast'):
                image_features = self.prepare_slowfast(
                    image_features, temporal_aggregation)
            else:

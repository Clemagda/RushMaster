...
import torch
from slowfast_llava.llava.model import *
from slowfast_llava.llava.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            if 'mpt' in model_name.lower():
                ...
                if rope_scaling_factor >= 2:
                    setattr(cfg_pretrained, "rope_scaling", {
                            "factor": float(rope_scaling_factor), "type": "dynamic"})
                    setattr(cfg_pretrained, "max_sequence_length",
                            4096 * rope_scaling_factor)
                    setattr(cfg_pretrained, "tokenizer_model_max_length",
                            4096 * rope_scaling_factor)
                model = LlavaLlamaForCausalLM.from_pretrained(
                    model_path,
                    low_cpu_mem_usage=True,
